    ...
    <section className="rounded-3xl bg-white/85 p-6 shadow-card backdrop-blur">
      <div className="flex items-center justify-between gap-4">
        <div>
          ...
          <p className="mt-2 text-sm text-slate-600">Every intermediate JSON artifact is stored locally for inspection and reuse.</p>
        </div>
        {job ? (
      ...
      {artifacts ? (
        <div className="mt-4 rounded-2xl bg-slate-50 px-4 py-3 text-sm text-slate-600">
          Deck status: {artifacts.deck_status}. Grounding warnings: {artifacts.grounding_status ?? 0}. Critic approved: {artifacts.critic_approval_status === null || artifacts.critic_approval_status === undefined ? "n/a" : artifacts.critic_approval_status ? "yes" : "no"}.
        </div>
      ) : null}
      <div className="mt-5 space-y-3">
        ...
        {!job ? <p className="text-sm text-slate-500">No artifacts yet.</p> : null}
      </div>
    </section>
  );
}
